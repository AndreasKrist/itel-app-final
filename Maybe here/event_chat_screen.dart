...
class _EventChatScreenState extends State<EventChatScreen>
  ...
  Stream<EventBan?> _getBanStream(String userId) {
    if (_banStream == null || _banStreamUserId != userId) {
      _banStreamUserId = userId;
      ...
      builder: (context) => SafeArea(
        child: SingleChildScrollView(
          ...
          children: [
            ...
            // Toggle working hours (staff only)
            if (isStaff)
              ListTile(
                leading: Icon(
                  event.disableWorkingHours ? Icons.lock_clock : Icons.lock_open,
                  color: event.disableWorkingHours ? Colors.orange : Colors.green,
                ),
                title: Text(event.disableWorkingHours ? 'Enable Working Hours' : 'Disable Working Hours'),
                subtitle: Text(
                  event.disableWorkingHours
                      ? 'Chat is currently available 24/7'
                      : 'Chat is restricted to business hours',
                  style: const TextStyle(fontSize: 12),
                ),
                onTap: () {
                  Navigator.pop(context);
                  _toggleWorkingHours(event);
                },
              ),
  ...
  Future<void> _toggleWorkingHours(Event event) async {
    try {
      await _eventService.updateEvent(
        eventId: widget.eventId,
        data: {'disableWorkingHours': !event.disableWorkingHours},
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(event.disableWorkingHours
                ? 'Working hours restriction enabled'
                : 'Working hours restriction disabled — chat available 24/7'),
            backgroundColor: Colors.green,
          ),
        );
      }
            ...
            children: [
              ...
              if (!isGuest)
                StreamBuilder<EventBan?>(
                  stream: _getBanStream(currentUser.id),
                  builder: (context, banSnapshot) {
                    ...
                    // Check working hours
                    if (!event.disableWorkingHours && !WorkingHoursHelper.isWithinWorkingHours()) {
                      return Container(
                        padding: const EdgeInsets.all(16),
                        decoration: BoxDecoration(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      'Live chat is unavailable outside of business hours.',
                                      style: TextStyle(
